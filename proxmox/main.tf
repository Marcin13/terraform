...
provider "proxmox" {
  ...
  pm_api_token_id = "terraform-prov@pve!id1234"
  # this is the full secret wrapped in quotes. don't worry, I've already deleted this from my proxmox cluster by the time you read this post
  pm_api_token_secret = "2b7ab584-8ddd-4193-a5e3-f780daf232d1"
...
resource "proxmox_vm_qemu" "vm-server" {
  ...
  desc = "Ubuntu 20.04 clean vm for Ansible"
  vmid = "10${count.index + 1}" #id for new created vm etc .101, 102, 103
  ...
  ciuser = "root"  # Override the default cloud-init user for provisioning.
  ...
  disk {
    ...
    type = "scsi"
    storage = "local-lvm"
    size = "26G" # set disk size here. leave it small for testing because expanding the disk takes time.
